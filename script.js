...
/* ============================================================
   Liquid-metal CTA — hand-written WebGL2 chromatic liquid-metal skin
   on the "Get in touch" nav button (no dependency). Wandering halo
   specular + cursor-reactive dent/highlight. Falls back to the plain
   button when WebGL2 is missing or the user prefers reduced motion.
   ============================================================ */
(() => {
...
float hash(vec2 p){ p = fract(p * vec2(123.34, 345.45)); p += dot(p, p + 34.34); return fract(p.x * p.y); }
float noise(vec2 p){
...
void main(){
  ...
  // domain-warped flow -> liquid metal heightfield
  vec2 q = vec2(fbm(pp * 3.5 + vec2(0.0, t)), fbm(pp * 3.5 + vec2(5.2, -t)));
  float h = fbm(pp * 3.5 + q * 1.6 + t * 0.6);

  // cursor dent
  float md = distance(uv, uMouse);
  h -= exp(-md * md * 22.0) * (0.18 + 0.55 * uHover);

  // normals from the heightfield
  float e = 0.003;
  float hx = fbm((pp + vec2(e, 0.0)) * 3.5 + q * 1.6 + t * 0.6);
  float hy = fbm((pp + vec2(0.0, e)) * 3.5 + q * 1.6 + t * 0.6);
  vec3 n = normalize(vec3((h - hx), (h - hy), e * 8.0));

  // reflected environment, split per channel for a chromatic sheen
  vec3 r = reflect(vec3(0.0, 0.0, -1.0), n);
  float g0 = r.y * 0.5 + 0.5 + h * 0.3;
  float cr = smoothstep(0.10, 0.95, g0 + 0.05 * sin(uTime * 0.5));
  float cg = smoothstep(0.12, 0.92, g0);
  float cb = smoothstep(0.10, 0.95, g0 - 0.05 * sin(uTime * 0.5));
  // dark liquid chrome so the white label stays readable
  vec3 metal = mix(vec3(0.04, 0.05, 0.07), vec3(0.30, 0.34, 0.42), vec3(cr, cg, cb));

  // wandering halo specular, pulled toward the cursor on hover
  vec2 halo = vec2(0.5 + 0.42 * sin(uTime * 0.6), 0.5 + 0.45 * cos(uTime * 0.8));
  halo = mix(halo, uMouse, uHover * 0.7);
  float spec = pow(max(0.0, 1.0 - distance(uv, halo) * 1.9), 5.0);

  // fresnel rim
  float fres = pow(1.0 - abs(n.z), 2.5);

  vec3 col = metal + spec * vec3(0.85, 0.90, 1.0) * 1.0 + fres * vec3(0.35, 0.45, 0.65) * 0.35;
  col *= (0.9 + 0.4 * uHover);
  frag = vec4(col, 1.0);
}`;
  ...
  const compile = (gl, type, src) => {
    ...
    if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
      console.warn("metal shader:", gl.getShaderInfoLog(s));
      return null;
    }
    return s;
  };
  ...
  buttons.forEach((btn) => {
    ...
    if (!gl) return; // fallback: leave the plain button
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      console.warn("metal link:", gl.getProgramInfoLog(prog));
      return;
    }

    // wrap the label so it sits above the canvas, then insert the canvas
    ...
    btn.addEventListener("pointerenter", () => (targetHover = 1));
